...
public class HttpServer {
        ...
        try {
            ...
            while (running) {
                ...
                if (requestURI.getPath().startsWith("/cliente")) {
                    outputLine = clientResponse(requestURI);
                } else if (requestURI.getPath().startsWith("/consulta")) {
                    outputLine = consultResponse(requestURI);
                } else {
                    outputLine = errorPage();
                }
    ...
    private static String consultResponse(URI requestURI) throws IOException {
        JsonObject response = HttpFacadeServer.getInstance().getReflectiveChatCommand(requestURI.getQuery());
    ...
    public static String clientResponse(URI requestURI) {
        return "HTTP/1.1 200 OK\n" +
                "Content-Type: text/html\n" +
                "\n" +
                "<!DOCTYPE html>\r\n" +
